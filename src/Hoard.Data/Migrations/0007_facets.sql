...
-- add will be on the wrong side of it: if the value would change because Hoard
...
--
-- ── `game_mode` is the one kind Hoard owns, and it is seeded here ────────────
--
...
-- target, and its slugs are Hoard's own vocabulary rather than anyone's ids.
--
...
-- Hoard's own game-mode vocabulary. Fixed ids; see the note above.
