...
public static class Apicalypse
{
    ...
    ///
    /// <para>Image IDs remain the durable CDN handles. Source dimensions,
    /// transparency and animation flags let backdrop selection evaluate an
    /// image before downloading. Artwork expands the current image_type name;
    /// the deprecated artwork_type field is not requested. Ratings remain
    /// attributed separately to users and critics. These fields stay off the
    /// isolated search and age-rating queries.</para>
    /// </summary>
    public static string Games(IEnumerable<long> igdbIds, int limit, int offset)
        => $"""
            fields name,summary,first_release_date,cover.image_id,cover.url,genres.name,themes.name,game_modes.name,player_perspectives.name,platforms.name,involved_companies.publisher,involved_companies.company.name,game_type.type,parent_game,version_parent,version_title,screenshots.image_id,screenshots.width,screenshots.height,screenshots.alpha_channel,screenshots.animated,artworks.image_id,artworks.width,artworks.height,artworks.alpha_channel,artworks.animated,artworks.image_type.name,rating,rating_count,aggregated_rating,aggregated_rating_count;
