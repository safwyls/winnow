...
public enum EpicConnection
{
    ...
    /// explanation and Hoard appearing to have forgotten them.
    /// </summary>
    Lapsed,
}

/// <summary>
/// The Stores panel: the first screen in Hoard that says out loud which sources
...
/// Hoard everything it can, <c>Amber</c> for something the user meant to be
...
public partial class StoresViewModel : ObservableObject
{
    ...
    /// </summary>
    public const string ConsoleSignInCommand = "dotnet run --project src/Hoard.App -- --epic-login";
    ...
    public string IntroMessage =>
        ...
        + "and none of them needs an account for Hoard to work. Two of them know things those "
        + "files don't.";
    ...
    public string SteamLocalMessage =>
        "Always on. Steam's own config files give Hoard the games on this machine, whether they're "
    ...
    public string SteamAddsMessage => SteamWebApiConfigured
        ? "Hoard also reads your full owned list from Steam, so games you've played on another PC "
    ...
    /// launch, where it looks exactly like a bug: the user signed in, and Hoard
    /// asks again.
    /// </summary>
    [ObservableProperty]
    ...
    public string EpicSessionNotPersistedMessage =>
        "This machine can't encrypt the session, so it's held for this run only. Hoard will ask you "
    ...
    public string EpicLocalMessage =>
        "Always on. The launcher's own catalog gives Hoard your owned Epic games and which of them "
    ...
    public string EpicSignInAddsMessage =>
        ...
        + "ago you played one still comes from Hoard watching it run, not from Epic.";
    ...
    public string EpicConsentPromiseMessage =>
        "Before anything opens, Hoard shows you what it will be holding: a credential with full "
    ...
    /// password is typed into a window Hoard opened, so the user's protection
    ...
    public string EpicConsoleAlternativeMessage =>
        "Rather not type your Epic password into a window Hoard opened? Sign in from a terminal "
        + "instead and Hoard never hosts the page at all:";
    ...
    public string EpicLapsedMessage =>
        "The session expired while Hoard was closed. Your Epic games are still here from the local "
    ...
    public string GogLocalMessage =>
        "Always on, and it is the whole story. Galaxy's own database gives Hoard your owned GOG "
    ...
    /// beside the other two would say Hoard is missing a feature here, and it is
    ...
    public string GogNoSignInMessage =>
        ...
        + "database already gives Hoard. A sign-in here would cost you a password and buy nothing.";
