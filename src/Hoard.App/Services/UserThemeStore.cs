...
public sealed class UserThemeStore : IDisposable
{
    ...
    /// </summary>
    public (IReadOnlyList<HoardTheme> Themes, IReadOnlyList<ThemeDiagnostic> Diagnostics) Load()
    {
        var themes = new List<HoardTheme>();
    ...
    public (string? FileName, ThemeDiagnostic? Problem) Export(HoardTheme theme)
    {
        try
        {
            if (!System.IO.Directory.Exists(Directory))
    ...
    private static string ExampleText()
    {
        ...
        body.AppendLine("// A Hoard theme. Copy this file, change the id, and edit.");
        body.AppendLine("//");
        ...
        body.AppendLine("// MEANS, and it may not spend one role's colour on a second job. Hoard warns");
        ...
        body.AppendLine("//");
        body.AppendLine("// The ones below are the values Hoard's own default theme was hand-tuned to");
        ...
        body.AppendLine();
        body.Append(ThemeJson.Export(HoardThemes.Default));
