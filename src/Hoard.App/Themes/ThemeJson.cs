...
public static class ThemeJson
{
    ...
    public static (HoardTheme? Theme, IReadOnlyList<ThemeDiagnostic> Diagnostics) Parse(
        string fileName, string text)
    {
        ...
        if (version != SchemaVersion)
        {
            Error(
                ...
                version > SchemaVersion
                    ? $"this file is written to version {version} and this build of Hoard reads version {SchemaVersion}. It is refused rather than read as best it can be, because a field that moved between versions would load at its default and give you a theme you did not write. Update Hoard, or set schemaVersion to {SchemaVersion} and check the fields against the example file."
        ...
        else if (HoardThemes.All.Any(t => string.Equals(t.Id, id, StringComparison.Ordinal)))
        {
            Error(
                "id",
    ...
    /// values by hand.</para>
    /// </summary>
    public static string Export(HoardTheme theme)
    {
    ...
    /// is applied by <c>HoardTheme.Tokens</c> from the transparency setting - so
        ...
        ThemeDefaultsDocument? defaults, Action<string, string> warn)
    {
        ...
        if (Clean(defaults.Backdrop) is { Length: > 0 } backdropId)
        {
            ...
            {
                backdrop = HoardBackdrops.ById(backdropId);
            }
            else
            {
                warn(
        ...
        if (Clean(defaults.Layout) is { Length: > 0 } layoutId)
        {
            ...
            {
                layout = HoardLayouts.ById(layoutId);
            }
            else
            {
                warn(
    ...
    private static ThemeDefaultsDocument? ExportDefaults(ThemeAppearanceDefaults? defaults)
    {
        ...
        return new ThemeDefaultsDocument
        {
            ...
            Backdrop = defaults.Backdrop is { } b ? HoardBackdrops.Id(b) : null,
            ...
            Layout = defaults.Layout is { } l ? HoardLayouts.Id(l) : null,
        };
    }
