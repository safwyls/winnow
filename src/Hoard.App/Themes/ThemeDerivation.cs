...
/// <summary>
/// Seeds plus proportions in, a complete <see cref="HoardTheme"/> out — and the
...
/// useful result.</b> The residuals are not scattered: they land on Hoard's
...
public static class ThemeDerivation
{
    ...
    /// against Hoard's own translucent chrome ink the law lands on 1.366 against
    ...
    /// template lists them, which is the order <see cref="HoardTheme"/> declares
    ...
    /// </summary>
    public static HoardTheme Compose(
        string id,
        string name,
        string reason,
        ThemeSeeds seeds,
    ...
    public static ThemeSeeds SeedsOf(HoardTheme theme) => new()
    {
        Ground = theme.Ground,
    ...
    /// them to guess.</para>
    /// </summary>
    public static ThemeShape Fit(HoardTheme theme)
    ...
    /// </summary>
    private static ThemeShape Refine(HoardTheme theme, ThemeShape raw)
    {
    ...
    private static ThemeShape RawFit(HoardTheme theme)
    {
    ...
    /// </remarks>
    public static Dictionary<string, Color> ResidualOverrides(HoardTheme theme, ThemeShape shape)
    {
        var seeds = SeedsOf(theme);
        var actual = ActualDerivedFields(theme);
    ...
    public static Dictionary<string, Color> ActualDerivedFields(HoardTheme theme)
