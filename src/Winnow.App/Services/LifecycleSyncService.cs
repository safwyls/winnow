...
{
    public const string ScheduleKey = "lifecycle.poll-attempts.v1";
    ...
    public async Task<int> SyncAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            Dictionary<long, DateTime> attempts;
            try { attempts = rawSchedule is null ? [] : JsonSerializer.Deserialize<Dictionary<long, DateTime>>(rawSchedule) ?? []; }
            catch (JsonException) { attempts = []; }
            var now = clock.GetUtcNow().UtcDateTime;
            var due = targets.Where(t => t.IgdbId.HasValue || t.SteamAppId is not null)
                .Where(t => !attempts.TryGetValue(t.ReleaseId, out var last) || last <= now.AddDays(-1))
                .OrderBy(t => attempts.GetValueOrDefault(t.ReleaseId)).ThenBy(t => t.ReleaseId)
            ...
            foreach (var target in due)
            {
                ...
                try
                {
                    if (target.IgdbId is { } id && await igdb.GetAsync(id, ct) is { } result)
                        await Append("igdb", id.ToString(System.Globalization.CultureInfo.InvariantCulture), result.ObservedAt, result.Signals, result.RawJson);
                }
                ...
                attempts[target.ReleaseId] = clock.GetUtcNow().UtcDateTime;
