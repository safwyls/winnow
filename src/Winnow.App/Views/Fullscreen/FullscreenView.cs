...
public sealed partial class FullscreenView : UserControl, IDisposable
{
    ...
    public FullscreenView(FullscreenContext context, bool preparing = false)
    {
        ...
        updateNotice.Bind(IsVisibleProperty, new Binding(nameof(ApplicationSettingsViewModel.ShowUpdateAction)) { Source = context.Shared.ApplicationSettings });
        ...
        var updateProgress = new StackPanel { Name = "FullscreenUpdateProgress", Spacing = 4, MinWidth = 220, HorizontalAlignment = HorizontalAlignment.Right };
        updateProgress.Bind(IsVisibleProperty, new Binding(nameof(ApplicationSettingsViewModel.CanCancelUpdate)) { Source = context.Shared.ApplicationSettings });
        var updating = FullscreenUi.Text("Updating", 24, "TextDim");
        updating.HorizontalAlignment = HorizontalAlignment.Right;
        updateProgress.Children.Add(updating);
        var downloadProgress = new ProgressBar { Name = "FullscreenUpdateProgressBar", Height = 4, Minimum = 0, Maximum = 100 };
        downloadProgress[!ProgressBar.ForegroundProperty] = new DynamicResourceExtension("Volt");
        downloadProgress[!ProgressBar.BackgroundProperty] = new DynamicResourceExtension("Line");
        downloadProgress.Bind(ProgressBar.ValueProperty, new Binding(nameof(ApplicationSettingsViewModel.UpdateProgress)) { Source = context.Shared.ApplicationSettings });
        AutomationProperties.SetName(downloadProgress, "Updating");
        downloadProgress.Bind(AutomationProperties.ItemStatusProperty, new Binding(nameof(ApplicationSettingsViewModel.UpdateStatus)) { Source = context.Shared.ApplicationSettings });
        updateProgress.Children.Add(downloadProgress);
        statusGroup.Children.Add(updateProgress);
