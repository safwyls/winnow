...
public static class ThemeAudit
{
    ...
    public static IReadOnlyList<ThemeDiagnostic> Inspect(WinnowTheme theme, string fileName)
    {
        ...
        // cover the art could be rather than against one reference colour.
        var artBacked = Colorimetry.WorstArtBackedContrast(theme);
        if (artBacked < Colorimetry.AaThreshold)
        {
            Warn(
                "seeds.surface",
                string.Create(
