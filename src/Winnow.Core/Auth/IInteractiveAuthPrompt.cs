...
/// not code, so <c>Winnow.Core</c> and the browser host both stay ignorant of
...
/// getting a code back.
///
/// <para><b>This is a contract only.</b> It lives in <c>Winnow.Core</c>, which
/// does no IO and references neither Avalonia nor WebView2, so that
/// <c>Winnow.Ingest.*</c> can depend on the SEAM without ever depending on a UI
/// framework (§5.1). The embedded-browser implementation lives in
/// <c>Winnow.Auth.WebView</c>; the console implementation lives in
/// <c>Winnow.App</c>. Neither is the "real" one and neither is a legacy path — a
...
public interface IInteractiveAuthPrompt
{
    ...
    ///
    /// <para>Winnow never sees the user's password: authentication happens on the
