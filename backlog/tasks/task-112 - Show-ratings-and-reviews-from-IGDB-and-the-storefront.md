...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Available ratings are shown in the details modal, each attributed to its source
- [x] #2 The number of ratings behind a score is shown, so a 9 from four people does not read like a 9 from four thousand
- [x] #3 A game with no rating data shows nothing rather than a zero or an empty scale
- [x] #4 Fetching is rate-limited, cached and soft-failing
- [x] #5 What each source provides, and what it does not, is recorded in docs/facet-provenance.md
<!-- AC:END -->
...
Reception became a line in Band 1 under year and publisher rather than a section of its own. Up to three figures in a fixed order - IGDB's own user body, IGDB's aggregation of external critics, Steam's reviewers - each with its source named and its count on the line, because a 9 from four people and a 9 from four thousand are different claims. The three are never blended and Winnow computes no verdict. Steam's own label is kept off the line and arrives with the percentage and the count on hover. A source with no figure writes no row in work_ratings, so absence is a property of the data; no figure at all means no line, never a zero and never an empty scale. Verified by GameReceptionViewModelTests (order, attribution, counts, the label on hover only, a zero count is no figure, nothing blends) and by WorkReceptionTests for the storage half. docs/facet-provenance.md already records both sources, their field paths, batching, cache, TTL and what each does not provide. Measured, not estimated: the three figures are 184 / 210 / 170px and sum to 564, so the line is a WrapPanel - two rows in the 420px column at the card's MinWidth, one row at 580. No new ink: the value takes Text and the attribution and count take TextDim, both already walked over all 256 greys; TextFaint measures 2.86 / 3.01 / 2.67 / 2.60 over the brightest cover and was refused, with a test pinning the refusal. Build succeeded, 0 warnings, 0 errors; 3373 / 152 / 82 passing.
