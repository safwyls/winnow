...
Remove the 520px height cap and prevent nested presenter scrolling; verify the complete menu renders without scrollbars at normal window size.
<!-- SECTION:PLAN:END -->
...
Follow-up: removed the inner ScrollViewer and its 520px height cap. The presenter now sizes to the full content, allows up to 460px width, disables horizontal scrolling, and uses vertical scrolling only if screen space constrains the popup. Real-font headless capture C:/Temp/winnow-display-no-scroll.png shows every option and the complete focus ring. Presenter scrollbar inspection reports both horizontal and vertical IsVisible=False. Three CoverPresentationTests passed; temporary diagnostics removed. Fullscreen uses its separate settings presentation and is unaffected.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Display preferences size to their full content without nested scrolling or a fixed height cap. Verified all options and checkbox focus ring in a rendered capture, both scrollbar controls invisible, and three targeted tests passing. Fullscreen remains on its separate settings layout.
