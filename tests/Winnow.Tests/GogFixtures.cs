...
internal sealed class GalaxyFixtureTree : IDisposable
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "winnow-gog-fixture-" + Guid.NewGuid().ToString("N"));
    ...
    /// This is done to a Winnow-owned copy only; the real store file is never
