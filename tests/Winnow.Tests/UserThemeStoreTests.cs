...
/// <c>%LOCALAPPDATA%\Winnow\themes</c>, which is a real folder on the machine
/// this runs on.</para>
/// </summary>
public class UserThemeStoreTests : IDisposable
{
    private readonly string _dir = Path.Combine(
        Path.GetTempPath(), "winnow-themes-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var store = new UserThemeStore(_dir);
        var (file, problem) = store.Export(WinnowThemes.Nightshift);
        ...
        Assert.Equal(WinnowThemes.Nightshift.Line, loaded.Line);
    ...
    {
        var store = new UserThemeStore(_dir);
        Assert.Equal("winnow.json", store.Export(WinnowThemes.Winnow).FileName);
        Assert.Equal("winnow-2.json", store.Export(WinnowThemes.Winnow).FileName);
    }
    ...
    public async Task Selecting_a_builtin_touches_no_other_setting()
    {
        ...
        Assert.True(service.WallTranslucent);
        Assert.Equal(WinnowBackdrops.Default, service.Backdrop);
        Assert.Equal(WinnowLayouts.Default, service.Layout);
    }
