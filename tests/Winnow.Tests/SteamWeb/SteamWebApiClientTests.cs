...
using Xunit;
using Fixture = Winnow.Tests.SteamWeb.SteamWebFixtures.OwnedGameFixture;
...
public class SteamWebApiClientTests
{
    ...
    public async Task A_descriptive_user_agent_identifies_the_traffic()
    {
        ...
        var request = Assert.Single(host.Handler.Requests);
        Assert.Contains("Winnow", request.UserAgent ?? string.Empty, StringComparison.Ordinal);
    }
    ...
    public void Registrations_defer_to_anything_already_in_the_container()
    {
        ...
        services.AddSingleton<Winnow.Core.Repositories.ISettingsRepository>(settings);
        services.AddSteamWebApi();

        using var provider = services.BuildServiceProvider();

        Assert.Same(cache, provider.GetRequiredService<ISteamWebMetadataCache>());
        Assert.Same(settings, provider.GetRequiredService<Winnow.Core.Repositories.ISettingsRepository>());
    }
    ...
    public async Task The_module_works_with_nothing_configured()
    {
        ...
        services.AddSingleton<Winnow.Core.Repositories.ISettingsRepository>(new InMemorySettingsRepository());
        services.AddSteamWebApi();
