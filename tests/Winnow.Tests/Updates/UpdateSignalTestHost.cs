...
{
    private readonly List<Winnow.Core.Domain.UpdateEvent> _written = [];

    public IReadOnlyList<Winnow.Core.Domain.UpdateEvent> Written => _written.ToArray();

    public Task<bool> UpsertAsync(Winnow.Core.Domain.UpdateEvent updateEvent, CancellationToken ct = default)
    {
