...
public sealed class WorkImageMetadataWriterTests : IDisposable
{
    ...
    {
        var workId = await new WorkRepository(_db.Factory).InsertAsync(new Work { Name = "Canned game" });
        var images = new WorkImageRepository(_db.Factory);
        var writer = new WorkReceptionWriter(images, new WorkRatingRepository(_db.Factory));
        var legacy = new IgdbGame(100440, "Canned game", null, null, null, [], [], [])
        {
            ArtworkImageIds = ["ar1"],
            ScreenshotImageIds = ["sc1"],
        };
        Assert.Equal(2, await writer.ApplyIgdbAsync(workId, legacy));
        ...
        Assert.Equal(2, await writer.ApplyIgdbAsync(workId, enriched));
        ...
        Assert.Equal(0, await writer.ApplyIgdbAsync(workId, enriched));
        Assert.Equal(0, await writer.ApplyIgdbAsync(workId, legacy));

        var corrected = enriched with { ArtworkImages = [enriched.ArtworkImages[0] with { Width = 4096 }] };
        Assert.Equal(1, await writer.ApplyIgdbAsync(workId, corrected));
        Assert.Equal(0, await writer.ApplyIgdbAsync(workId, corrected));
    }
}
