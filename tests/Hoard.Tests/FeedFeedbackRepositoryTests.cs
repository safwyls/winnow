...
/// the launch-endorsement join counts only sessions Hoard itself launched,
...
public class FeedFeedbackRepositoryTests
{
    ...
    [Fact]
    public async Task Endorsements_count_only_hoard_launched_sessions_inside_the_window()
    {
        using var db = new TempDatabase();
        var repo = new FeedFeedbackRepository(db.Factory);
        ...
        // Hoard-launched, but ten days after the surfacing: by then the launch
