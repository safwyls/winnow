...
`moonlighter-catalog-entry.json` retains the public identity, releaseInfo and category fields from the copied launcher catalog (2026-09-06). It contains no account identifiers, credentials or user paths. The regression test encodes it as catcache.bin in a temporary tree and verifies the persisted launch URI remains the one Epic accepted.
